...
Phase expectations (cumulative):
  ...
  After Phase 4: + multiturn_coherence (phase-own skill; prior buckets stay required)
  After Phase 5: + toolcall_basic (phase-own skill; file eval if suite bucket missing)
  After Phase 6: + agentic_chain (phase-own skill; file eval if suite bucket missing)
  After Phase 7 (6.2): + docid_bind (rank-1 id from a multi-hit catalog; math advisory)
  After Phase 8 (6.3): + toolresult_ground (copy facts from get_doc text; math advisory)
...
PHASE_REQUIREMENTS = {
    ...
    4: {
        ...
        "context_citation": {"min_pass_rate": 50.0, "description": "Context-grounded answer + citation tags"},
        "multiturn_coherence": {"min_pass_rate": 50.0, "description": "Carry facts / switch / clarify across turns"},
    },
    5: {
        "format_strict": {"min_pass_rate": 90.0, "description": "Tags and stopping"},
        ...
        "multiturn_coherence": {"min_pass_rate": 45.0, "description": "Carry facts / switch / clarify across turns"},
        "toolcall_basic": {"min_pass_rate": 50.0, "description": "Emit a valid workspace toolcall"},
    },
    6: {
        "format_strict": {"min_pass_rate": 90.0, "description": "Tags and stopping"},
        "echo_basic": {"min_pass_rate": 70.0, "description": "Basic echo/repeat"},
        ...
        "multiturn_coherence": {"min_pass_rate": 40.0, "description": "Carry facts / switch / clarify across turns"},
        "toolcall_basic": {"min_pass_rate": 45.0, "description": "Emit a valid workspace toolcall"},
        "agentic_chain": {"min_pass_rate": 40.0, "description": "Multi-step tool chain after a tool result"},
    },
    7: {
        "format_strict": {"min_pass_rate": 90.0, "description": "Tags and stopping"},
        "echo_basic": {"min_pass_rate": 70.0, "description": "Basic echo/repeat"},
        ...
        "toolcall_basic": {"min_pass_rate": 45.0, "description": "Emit a valid workspace toolcall"},
        "agentic_chain": {"min_pass_rate": 40.0, "description": "Multi-step tool chain after a tool result"},
        "docid_bind": {"min_pass_rate": 70.0, "description": "Copy rank-1 doc_id from a multi-hit search catalog"},
    },
    8: {
        "format_strict": {"min_pass_rate": 90.0, "description": "Tags and stopping"},
        "echo_basic": {"min_pass_rate": 70.0, "description": "Basic echo/repeat"},
        ...
        "agentic_chain": {"min_pass_rate": 40.0, "description": "Multi-step tool chain after a tool result"},
        "docid_bind": {"min_pass_rate": 70.0, "description": "Copy rank-1 doc_id from a multi-hit search catalog"},
        "toolresult_ground": {"min_pass_rate": 70.0, "description": "Copy facts from get_doc toolresult into the answer"},
        "search_miss": {"min_pass_rate": 60.0, "description": "Abstain when search returns no matching documents"},
    },
}

# Failures here are logged, never a hard gate fail, never a math remediation.
STRUCTURAL_ADVISORY_BUCKETS = {
    7: {"regression_basic"},
    8: {"regression_basic"},
}

# Phase-own skills that live in JSONL until they have a suite bucket.
# Schema per line: {"prompt": "...", "expected_contains": ["..."], "category": "..."}
CAPABILITY_FILE_BUCKETS = {
    "toolcall_basic": PROJECT_ROOT / "data" / "eval_capability" / "toolcall_basic.jsonl",
    "agentic_chain": PROJECT_ROOT / "data" / "eval_capability" / "agentic_chain.jsonl",
    "docid_bind": PROJECT_ROOT / "data" / "eval_capability" / "docid_bind.jsonl",
    "toolresult_ground": PROJECT_ROOT / "data" / "eval_capability" / "toolresult_ground.jsonl",
    "search_miss": PROJECT_ROOT / "data" / "eval_capability" / "search_miss.jsonl",
}


def _unwrap_model(loaded):
    """load_model may return a GPT or a (model, ...) tuple depending on call site."""
...
def run_gate(
    ...
    eval_results = run_evaluation(
        ...
        system_prompt_preset=(
            "chat" if phase in (3, 4) else "agentic" if phase in (5, 6, 7) else None
        ),
    )

    for bucket_name, path in CAPABILITY_FILE_BUCKETS.items():
        if bucket_name not in requirements:
            continue
        if eval_results.get("buckets", {}).get(bucket_name):
            continue
        if not path.exists():
            print(f"\n  [WARN] capability file missing for {bucket_name}: {path}")
            continue
        print(f"\n  Running phase-own capability eval {bucket_name} from {path.name}...")
        file_result = _run_capability_file_eval(model_name, str(path), max_new_tokens, verbose)
        eval_results.setdefault("buckets", {})[bucket_name] = file_result
    ...
    for bucket_name, req in requirements.items():
        ...
        advisory = bucket_name in STRUCTURAL_ADVISORY_BUCKETS.get(phase, set())
        if status == "FAIL" and advisory:
            status = "STRUCTURAL"
        passed = status in ("PASS", "STRUCTURAL")
        icon = "PASS" if status == "PASS" else ("STRUCTURAL" if status == "STRUCTURAL" else "FAIL")
        ...
        if status == "MISSING":
            ...
            print(
                f"  [STRUCTURAL] {bucket_name}: {actual_rate:.1f}% "
                f"({bucket_data['passed']}/{total}) (threshold: {min_rate:.0f}%, "
                f"advisory — do not remediate) - {desc}"
            )
        elif total == 0:
            print(
...
def _run_capability_file_eval(
    model_name: str,
    jsonl_path: str,
    max_new_tokens: int,
    verbose: bool,
) -> Dict:
    """Score a phase-own capability JSONL into a suite-shaped bucket."""
    from core import load_model

    model = _unwrap_model(load_model(model_name))
    model.eval()
    ...
    for item in items:
        ...
        generated = output[len(prompt_text):] if output.startswith(prompt_text) else output
        gen_l = generated.lower()
        ok_any = (not expected) or any(kw.lower() in gen_l for kw in expected)
        ok_all = all(kw.lower() in gen_l for kw in expected_all)
        ok_forbid = not any(kw.lower() in gen_l for kw in forbidden)
        ok = ok_any and ok_all and ok_forbid
        details.append({
            "name": name,
            "passed": ok,
            "expected": expected,
        ...
        if ok:
            ...
            failed += 1
            print(
                f"  [FAIL] {name}: expected_contains={expected} "
                f"expected_all={expected_all} forbidden={forbidden} "
                f"got {generated[:80]!r}"
            )
...
def _run_ood_eval(
    ...
    for item in eval_items:
        ...
        if hasattr(model, "generate_text"):
            ...
            output = model.generate(prompt_text, max_new_tokens=max_new_tokens, temperature=0.0, top_k=0, top_p=1.0)
        generated = output[len(prompt_text):] if output.startswith(prompt_text) else output
...
def main():
    ...
    parser.add_argument("--phase", type=int, required=True, choices=[1, 2, 3, 4, 5, 6, 7, 8],
                        help="Current SFT phase (1-6, 7 for 6.2 doc-id bind, 8 for 6.3 toolresult ground)")
    ...
    if args.phase == 7 and args.max_new_tokens == 64:
        max_new = 128
    if args.phase == 8 and args.max_new_tokens == 64:
        max_new = 192
