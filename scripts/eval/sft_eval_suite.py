...
K) multiturn_coherence (Phase 4+ capability):
   - Planted facts in earlier turns; last user turn asks only about those facts
   - Held-out names/codes (not in generate_multiturn_sft.py topics)
   - Also: clarification, topic-switch, DE hold, assistant_context carry
...
from core.system_prompts import CONVERSATION_SYSTEM_PROMPT, resolve_system_prompt
...
def make_history_prompt(messages: List[Tuple[str, str]], no_system: bool = None) -> str:
    """Build a multi-turn prefix that ends at an open assistant turn.

    messages: (role, content) with role in user / assistant / assistant_context.
    The last message must be user. Matches packer tag order (no per-turn EOT).
    """
    if not messages or messages[-1][0] != "user":
        raise ValueError("make_history_prompt requires a trailing user turn")
    use_no_system = no_system if no_system is not None else _NO_SYSTEM_PROMPT
    parts: List[str] = []
    if not use_no_system:
        parts.append(f"<myPT_system>{SYSTEM_PROMPT}</myPT_system>")
    for role, content in messages:
        if role == "user":
            parts.append(f"<myPT_user>{content}</myPT_user>")
        elif role == "assistant":
            parts.append(f"<myPT_assistant>{content}</myPT_assistant>")
        elif role == "assistant_context":
            parts.append(f"<myPT_assistant_context>{content}</myPT_assistant_context>")
        else:
            raise ValueError(f"unsupported role {role!r}")
    parts.append("<myPT_assistant>")
    return "".join(parts)


def rebuild_standard_eval_prompts() -> None:
    """Rebuild buckets A-D and F-J using the current SYSTEM_PROMPT / _NO_SYSTEM_PROMPT.

    Module-level lists are filled at import with the conversation prompt. Phase 3-4
    eval must call this after resolve_system_prompt(preset='chat').
    Hierarchy prompts keep their own explicit system strings.
    """
    global FORMAT_PROMPTS, ECHO_PROMPTS, ANTI_ECHO_PROMPTS, REGRESSION_PROMPTS
    global INJECTION_PROMPTS, ABSTAIN_CONTEXT_PROMPTS, STRICT_FORMAT_PROMPTS, CONTEXT_CITATION_PROMPTS
    global MULTITURN_PROMPTS

    FORMAT_PROMPTS = [
        ("format_say_ok", make_prompt("Say OK."), None),
    ...
    ANTI_ECHO_PROMPTS = [
        ("anti_blurpix", make_prompt('What is "Blurpix"?'), "Blurpix"),
    ...
    # Held-out nonce facts — do not reuse generate_multiturn_sft.py topics.
    MULTITURN_PROMPTS = [
        (
            "mt_name_carry",
            make_history_prompt([
                ("user", "My cat is named Zephir."),
                ("assistant", "Noted, your cat is named Zephir."),
                ("user", "What did I name my cat?"),
            ]),
            "Zephir",
        ),
        (
            "mt_code_distract",
            make_history_prompt([
                ("user", "Remember this code: NEX-918."),
                ("assistant", "I will remember NEX-918."),
                ("user", "Thanks. What is 1 + 1?"),
                ("assistant", "2"),
                ("user", "What was the code I gave you?"),
            ]),
            "NEX-918",
        ),
        (
            "mt_city_carry",
            make_history_prompt([
                ("user", "I live in the city of Keldara."),
                ("assistant", "You live in Keldara."),
                ("user", "Which city do I live in?"),
            ]),
            "Keldara",
        ),
        (
            "mt_count_carry",
            make_history_prompt([
                ("user", "I have exactly 17 wibble stones."),
                ("assistant", "You have 17 wibble stones."),
                ("user", "How many wibble stones do I have?"),
            ]),
            "17",
        ),
        (
            "mt_clarify_pick",
            make_history_prompt([
                ("user", "I can use red paint or teal paint."),
                ("assistant", "You can use red or teal."),
                ("user", "I will use teal. Which color did I pick?"),
            ]),
            "teal",
        ),
        (
            "mt_topic_switch",
            make_history_prompt([
                ("user", "Tell me about my cat Zephir."),
                ("assistant", "Zephir is your cat."),
                ("user", "New question: what color is a ripe banana?"),
            ]),
            "yellow",
        ),
        (
            "mt_de_name",
            make_history_prompt([
                ("user", "Mein Hund heißt Brio."),
                ("assistant", "Dein Hund heißt Brio."),
        ...
        (
            "mt_context_carry",
            make_history_prompt([
                ("user", "I opened a support ticket."),
                ("assistant", "Okay."),
                ("assistant_context", "The ticket id is WRN-440."),
                ("user", "What is the ticket id?"),
            ]),
            "WRN-440",
        ),
    ]
...
def run_evaluation(
    ...
    Args:
        ...
        system_prompt_preset: conversation (default), chat (Phase 3-4), or agentic.
    
    Returns dict with per-bucket results and overall pass/fail.
    """
    global SYSTEM_PROMPT, OPERATOR_PROMPTS
    SYSTEM_PROMPT = resolve_system_prompt(preset=system_prompt_preset)
    rebuild_standard_eval_prompts()
    OPERATOR_PROMPTS = build_operator_prompts(use_val_templates=use_val_templates)
    
    print(f"Loading model: {model_name}")
    print(f"  System prompt: {'OMITTED' if _NO_SYSTEM_PROMPT else 'included'}")
    if not _NO_SYSTEM_PROMPT:
        print(f"  System prompt preset: {system_prompt_preset or 'conversation'}")
    ...
    results = {
        ...
        "system_prompt_preset": system_prompt_preset or "conversation",
    ...
    if not operators_only:
        ...
        # Bucket K: Multi-turn coherence (Phase 4 capability)
        print("\n📋 Bucket K: Multi-turn Coherence")
        bucket_k = {"passed": 0, "failed": 0, "details": []}
        for name, prompt, expected in MULTITURN_PROMPTS:
            gen = generate(prompt)
            passed, reason = check_regression(gen, expected)
            bucket_k["details"].append({
                "name": name, "passed": passed, "reason": reason,
                "expected": expected, "generated": gen[:180],
            })
            if passed:
                bucket_k["passed"] += 1
                if verbose:
                ...
                bucket_k["failed"] += 1
                print(f"  ❌ {name}: {reason}")
        results["buckets"]["multiturn_coherence"] = bucket_k
...
def main():
    ...
    parser.add_argument("--system_prompt_preset", type=str, default=None,
                        choices=["conversation", "chat", "agentic"],
                        help="System prompt from core.system_prompts (default: conversation). "
                             "Use chat for Phase 3-4 models.")
    ...
    results = run_evaluation(
        ...
        use_val_templates=args.use_val_templates,
    )
