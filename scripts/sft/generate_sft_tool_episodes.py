...
Per the tool-sequence generator spec:
...
# Phase 5 lock: one agentic system, 4k-safe cites. Do not mix short tool prompts.
SYSTEM_PROMPTS = [AGENTIC_STANDARD_PROMPT]
LIST_DOCS_MAX = 10
GETDOC_CITE_TOKENS = 480
ANSWER_CONTENT_TOKENS = 220

_AGENT_SUMMARIES = None


def _load_agent_summaries() -> Dict[str, Dict[str, Any]]:
    global _AGENT_SUMMARIES
    if _AGENT_SUMMARIES is not None:
        return _AGENT_SUMMARIES
    _AGENT_SUMMARIES = {}
    path = Path("data/sft_phase5_intermediate/doc_agent_summaries.jsonl")
    if path.exists():
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if fn:
                _AGENT_SUMMARIES[fn] = row
    return _AGENT_SUMMARIES


def _doc_summary(doc, lang: str) -> str:
    row = _load_agent_summaries().get(getattr(doc, "filename", ""))
    if row:
        key = "summary_de" if lang == "de" else "summary_en"
        text = (row.get(key) or row.get("summary_en") or "").strip()
        if text:
            return text
    return structured_summary(getattr(doc, "text", "") or "", 400, lang)


def _clip_doc(text: str, max_tokens: int) -> str:
    return clip_indexed_for_context(text or "", max_tokens)
...
def gen_search_getdoc_answer(
    ...
    clipped = _clip_doc(doc.text, GETDOC_CITE_TOKENS)
    getdoc_result = {
        "doc_id": doc_id, "title": title,
        "text": clipped, "length": len(doc.text),
    }

    content = _clip_doc(doc.text, ANSWER_CONTENT_TOKENS)
...
def gen_list_select_summarize(
    ...
    others = [d for d in documents if d is not doc]
    shown = [doc] + rng.sample(others, k=min(LIST_DOCS_MAX - 1, len(others)))
    rng.shuffle(shown)
    list_result = {
        "documents": [
        ...
        "messages": [
            ...
            {"role": "assistant",
             "content": rng.choice(tpl_a).format(
                 count=len(shown), filename=doc.filename, summary=summary),
             "cite": doc.filename},
        ],
        "language": lang,
    }
