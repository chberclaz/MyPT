...
def generate_summarize_example(
    ...
    summary = structured_summary(doc.text, max_length=400, lang=lang)
    if len(summary) < 40:
        summary = structured_summary(doc.text[:2000], max_length=400, lang=lang)
...
def generate_wrong_tool_example(
    ...
    if scenario["correct_tool"] == "workspace.search":
        ...
        answer_text = rng.choice(tpl["summarize_a"]).format(
            filename=doc.filename, summary=structured_summary(doc.text, max_length=400, lang=lang)
        )
    else:
...
def main():
    ...
    pattern_weights = [
        ("search",       0.18 * tool_budget),
        ("list_docs",    0.10 * tool_budget),
        ("get_doc",      0.16 * tool_budget),
        ("summarize",    0.30 * tool_budget),
        ("multi_step",   0.14 * tool_budget),
        ("no_results",   0.12 * tool_budget),
