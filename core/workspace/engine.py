...
class WorkspaceEngine:
    ...
    def _doc_from_index(self, doc_id: str) -> Optional[Document]:
        """Resolve a document that exists in the RAG index but not the txt/md scan (e.g. PDFs)."""
        if self._retriever is None:
            return None
        for meta in self._retriever.metadata:
            source = meta.get("source") or {}
            path = source.get("file") or ""
            if path and self._generate_doc_id(path) == doc_id:
                p = Path(path)
                return Document(doc_id=doc_id, title=p.stem, path=path)
        return None

    def _text_from_index(self, doc_id: str) -> Optional[str]:
        """Concatenate indexed chunks for a doc_id (PDFs and other non-plaintext files)."""
        if self._retriever is None:
            return None
        parts = []
        for meta in self._retriever.metadata:
    ...
    def get_doc(self, doc_id: str) -> Optional[Document]:
        ...
        """
        return self._documents.get(doc_id) or self._doc_from_index(doc_id)
    ...
    def get_doc_text(self, doc_id: str) -> Optional[str]:
        ...
        return self._text_from_index(doc_id)
