...
class GPT(nn.Module):
    ...
    def compile_for_inference(self, mode: str = "default"):
        ...
        Compiles the module (not self.forward) so __call__ hooks still run.
        ...
        try:
            compiled = _torch.compile(self, mode=mode)
            print("Model compiled successfully!")
            return compiled
        except Exception:
            print("Note: Triton not available, using eager mode fallback")
            return _torch.compile(self, backend="aot_eager")
    ...
    def generate(self, prompt, max_new_tokens, temperature=0.8, top_k=50, top_p=0.95,
                ...
                use_default_stop_tokens: bool = True, no_repeat_ngram: int = 0,
                ban_toolcall_after_doc_body: bool = False, return_ids: bool = False):
        """
        ...
        Args:
            ...
            no_repeat_ngram: Ban repeating n-grams of this size (0=disabled)
            ban_toolcall_after_doc_body: If the last toolresult already has
                document text/summary, mask ``<myPT_toolcall>`` even under
                greedy. Not answer stitch — the model still writes the facts.
        """
        device = self.config.device
        ...
        # STRICT_GREEDY: when temperature <= 0, skip sampling filters and do
        # pure argmax. Exception: decode-stop after a get_doc/summarize body
        # (ban <myPT_toolcall>) — otherwise greedy repeats retrieve+toolcall.
        STRICT_GREEDY = (temperature is None) or (float(temperature) <= 0.0)

        banned_ids: list[int] = []
        if ban_toolcall_after_doc_body:
            from core.agent.parsing import prompt_has_doc_body

            if prompt_has_doc_body(prompt):
                tid = (getattr(self.tokenizer, "special_tokens", None) or {}).get(
                    "myPT_toolcall_open"
                )
                if tid is not None:
                    banned_ids.append(int(tid))
        banned_idx = (
            torch.tensor(banned_ids, device=device, dtype=torch.long)
            if banned_ids
            else None
        )
        ...
        for _ in range(max_new_tokens):
            ...
            if banned_idx is not None:
                ...
                step_logits.index_fill_(1, banned_idx, float("-inf"))
            # STRICT_GREEDY: argmax (decode-stop mask already applied)
            if STRICT_GREEDY:
                idx_next = torch.argmax(step_logits, dim=-1, keepdim=True)
            else:
                base_logits = step_logits  # fp16/fp32 model dtype
                work_logits = step_logits.float()  # fp32 for filtering/sampling stability
                ...
                # Do not penalize special tokens: the system prompt already contains
                # <myPT_toolcall> as a format example, and that token must stay
                # samplable on the first assistant turn.
                if use_rep and recent_n > 0 and recent_len > 0:
                    ...
                    special_ids = getattr(self.tokenizer, "special_tokens", None) or {}
                    skip = {int(v) for v in special_ids.values() if v is not None}
                    if skip:
                        keep = [int(t) for t in uniq.tolist() if int(t) not in skip]
                        if not keep:
                            uniq = uniq[:0]
                        else:
                            uniq = torch.tensor(keep, dtype=uniq.dtype, device=uniq.device)
                    if uniq.numel() > 0:
