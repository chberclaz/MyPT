...
def special_tag_pattern():
    """Regex matching every packer tag string, longest-first."""
    import re

    tokens = sorted(SPECIAL_TOKEN_STRINGS.values(), key=len, reverse=True)
    return re.compile("|".join(re.escape(t) for t in tokens))


def strip_special_tag_strings(text: str) -> str:
    """Remove literal myPT tags from untrusted text (RAG ingest / user docs)."""
    if not text:
        return text
    return special_tag_pattern().sub("", text)
