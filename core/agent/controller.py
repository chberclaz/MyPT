...
from .parsing import (
    ...
    render_toolresult,
    truncate_toolresult,
    bind_doc_id_from_search,
    history_has_get_doc_body,
    is_collapsed_generation,
...
class AgentController:
    ...
    def __init__(
        ...
        self._recovered_collapse = False
        ...
        for step in range(max_steps):
            ...
            if has_toolcall(generated):
                ...
                if toolcall:
                    ...
                    exec_args = dict(toolcall.arguments or {})
                    if (
                        history_has_get_doc_body(history)
                        and toolcall.name == "workspace.get_doc"
                    ):
                        result = {
                            "error": "document_already_retrieved",
                            "hint": "Quote the previous get_doc toolresult. Do not call get_doc again.",
                        }
                        if verbose:
                            print("    [GET_DOC LOOP GUARD] document already in history")
                    else:
                        try:
                            if toolcall.name in ("workspace.get_doc", "workspace.summarize") and self._last_search_hits:
                                bound_id, reason = bind_doc_id_from_search(
                                    generated,
                                    exec_args.get("doc_id"),
                                    self._last_search_hits,
                                )
                                if bound_id and bound_id != exec_args.get("doc_id"):
                                    if verbose:
                                        print(
                                            f"    [DOC_ID BOUND] {exec_args.get('doc_id')} -> "
                                            f"{bound_id} ({reason})"
                                        )
                                    exec_args["doc_id"] = bound_id
                                    exec_args.pop("title", None)
                            result = self.tools.execute(toolcall.name, exec_args)
                            if toolcall.name == "workspace.search":
                                self._last_search_hits = list(result.get("documents") or [])
                            elif toolcall.name == "workspace.list_docs":
                                self._last_search_hits = list(result.get("documents") or [])
                            if AUDIT_AVAILABLE:
                    ...
                    _debug_tool_call(toolcall.name, exec_args, result, verbose)
                    ...
                    # Pretty-print search so each hit's doc_id sits *below*
                    # its snippet. Compact dumps put hit-2's id right after
                    # hit-1's content; 750M copies that next hex string.
                    dumps_kw = {"ensure_ascii": False}
                    if toolcall.name == "workspace.search":
                        dumps_kw["indent"] = 2
                    history.append({
                        "role": "toolresult",
                        "content": json.dumps(result, **dumps_kw),
                        "name": toolcall.name,
                    })
            ...
            # No toolcall. If the model dumped a think/[1][2][3]… template,
            # recover: get_doc the top hit, or answer from snippets (PDFs).
            if (
                is_collapsed_generation(generated)
                and self._last_search_hits
                and not self._recovered_collapse
            ):
                self._recovered_collapse = True
                top = self._last_search_hits[0]
                doc_id = top.get("doc_id")
                engine = getattr(self.tools, "engine", None)
                can_read = bool(doc_id and engine and engine.get_doc(doc_id))
                if can_read and step + 1 < max_steps:
                    if verbose:
                        print(f"\n  [COLLAPSE RECOVER] auto get_doc {doc_id}")
                    result = self.tools.execute("workspace.get_doc", {"doc_id": doc_id})
                    result = truncate_toolresult(result, self.max_result_chars)
                    tool_calls.append({
                    ...
                    continue
                fallback = fallback_answer_from_hits(self._last_search_hits)
                if fallback:
                    if verbose:
                        print("\n  [COLLAPSE RECOVER] extractive answer from search hits")
                    generated = fallback
