...
Interactive agent chat (PyTorch GOLD **or** GGUF):

```bash
python scripts/workspace_chat.py --list-models
python scripts/workspace_chat.py \
    --model_name phase6_3_ground_gold/mypt-q4_k_m.gguf \
    --workspace_dir workspace/ \
    --verbose
```

One-shot (same JSON as `workspace_rag.py`):

```bash
python scripts/workspace_chat.py \
    --model_name phase6_3_ground_gold/mypt-q4_k_m.gguf \
    --query "Which documents exist?" --json
```

### workspace_rag.py

Non-interactive RAG for other applications. One JSON object on stdout:

```bash
python scripts/workspace_rag.py \
    --model_name phase6_3_ground_gold/mypt-q4_k_m.gguf \
    --query "Summarize the handbook"
```

Python: `from core.agent import workspace_ask`. See [`export/README.md`](../../export/README.md).
...
- [Special Tokens](../model/SPECIAL_TOKENS.md) - Token definitions
- Phase 5 / Phase 3 training notes: [docs/sft/phases/PHASE5_TOOLCALL.md](../sft/phases/PHASE5_TOOLCALL.md), [docs/sft/phases/PHASE3_CHAT.md](../sft/phases/PHASE3_CHAT.md)
