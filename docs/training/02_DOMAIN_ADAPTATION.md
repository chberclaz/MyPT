...
Curriculum stage **2 of 3** before SFT. This is **continued pretrain**, not SFT Phase 2 (operators). Build the corpus first: [02_DOMAIN_CORPUS.md](02_DOMAIN_CORPUS.md). Next: [03_CONTEXT_EXTENSION.md](03_CONTEXT_EXTENSION.md). Map: [README.md](README.md).

Worked examples below may still name archived 1024-era presets (`configs/base/archiv/`). On the current spine, init from the unified GOLD and keep `eval_dataset_dir` pointed at a general holdout so you can see catastrophic forgetting.
