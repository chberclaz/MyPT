...
Curriculum stage **3 of 3** before SFT. Previous: [02_DOMAIN_ADAPTATION.md](02_DOMAIN_ADAPTATION.md). Then SFT: [../sft/README.md](../sft/README.md). Map: [README.md](README.md).
...
> **Dataset:** HuggingFace QA sources + pre-training general text, episode-indexed with packing. Measured token totals are not published.  
> **Training:** Continued pre-training, `rope_scale=4.0`. LR and step count for this project's run are not published.  
> **Input checkpoint:** `checkpoints/GOLD_unified_v1` (pre-trained at 1024 context)  
> **Date:** February 2026

**1.4B reuse:** inference stays **4096**. Either PI (`rope_scale=4.0`) after 1024 pretrain, or native 4096 pretrain. 40% QA during PI still strengthens retrieval heads. Pretrain corpus **#1** (extra 8B / 14B rebuild) is induction + retrieval — see [SCALE_1_4B.md](../sft/SCALE_1_4B.md).
...
2. Fine-tune on diverse text at the new context length (step count and token budget for this project are not published)
...
| `max_iters`              | (set in config) | Optimizer steps. Size to ~1 epoch of the combined QA+general dataset. Measured totals are not published. |
...
| `eval_interval`          | 500         | Evaluate on this interval; scale the number of evals to your `max_iters`. |
...
Warmup then cosine decay. Peak LR and step count for this project's run are not published.

### Effective Batch Size Calculation

```
micro_batch     = batch_size * block_size
effective_batch = micro_batch * grad_accum
total_tokens    = effective_batch * max_iters
```

Measured batch, step, and token totals for this project are not published.
...
- Training tokens / wall-clock / perplexity deltas for this project's run are not published
