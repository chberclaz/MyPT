...
Curriculum stage **1 of 3** before SFT. Next: [02_DOMAIN_CORPUS.md](02_DOMAIN_CORPUS.md). Map: [README.md](README.md).
...
**1.4B reuse:** keep the **category jobs** and LLaMA-2 style. Phase 1 token floor **≥ 14B unique**. Either **keep this 6B and add ~8B different data**, or **rebuild 14B from scratch**. **#1 for the extra 8B / new 14B:** induction + retrieval head mass (code exact-copy + extractive Q&A), two-stage curriculum. 750M 6.3 showed SFT cannot substitute for those circuits. Do not only re-epoch these shards. See [SCALE_1_4B.md](../sft/SCALE_1_4B.md) § Phase 1 base corpus. Do not init 1.4B from this checkpoint.
