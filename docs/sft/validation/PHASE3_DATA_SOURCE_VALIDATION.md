...
- Mitigation: keep the ratio in a maintenance band and rely on NOVEL eval templates.
...
- Mitigation: keep a low maintenance ratio, monitor the regression bucket.
...
| 6 | Operators + anti-echo as maintenance, not capability drivers | Implemented | `build_phase3_dataset.py` defaults — exact percents are not published |
