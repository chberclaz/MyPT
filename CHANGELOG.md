...
## [0.6.0.0] - 2026-09-01 — 750M agentic RAG curriculum closed

First time **all SFT phases (3–6.3)** for the ~750M agentic RAG PoC are trained, gated, and documented with shortcomings. Autopilot loop for this model is done. Do not start 1.4B from this tag.

### Added

- Thick phase specs (decisions, mix weights, why) for Phases 3–6.2 / 6.3; unified-P6 fallback spec; 1.4B from-scratch plan (`docs/sft/SCALE_1_4B.md`).
- Extra ~8B / 14B rebuild **#1:** induction + retrieval heads (code exact-copy + extractive Q&A).
- Runtime: lexical empty search catalog; rank-1 `doc_id` binder; refuse a second `get_doc` after a document body is in history (error toolresult, not answer stitch).

### 750M GOLD checkpoints

Curriculum phases 3–6.3 were trained and gated. Checkpoint names and measured scores are not published.

### Documented shortcomings (structural on 750M)

Known structural limits (math corpus, long tool chains, copy-vs-retrieve after get_doc) are recorded in the private tuning log. Do not treat them as unpublished benchmark claims here.
