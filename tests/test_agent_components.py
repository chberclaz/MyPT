...
def test_parsing():
    ...
    from core.workspace.tools import search_match_terms, lexical_overlap

    assert search_match_terms('search for "piedpiper"') == ["piedpiper"]
    assert "who" not in search_match_terms("who is emanuela Kyritsis?")
    assert search_match_terms("who is emanuela Kyritsis?") == ["emanuela", "kyritsis"]
    assert lexical_overlap("piedpiper", "INSTALL.md docker pip") == 0
    assert lexical_overlap("Christian Berclaz", "Mit freundlichen Grüssen, Christian Berclaz") >= 1
    print("[OK] search miss filter: no lexical overlap → 0")
