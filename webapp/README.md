...
- Model selection — PyTorch checkpoint folders **and** GGUF files (`checkpoints/<name>/*.gguf`, shown as `<name>/mypt-q4_k_m.gguf`)

To export a GOLD checkpoint and pick the GGUF in this dropdown, see [`export/README.md`](../export/README.md). Agentic mode runs `AgentController` against llama.cpp; LM Studio / Ollama will not execute `<myPT_toolcall>`.
