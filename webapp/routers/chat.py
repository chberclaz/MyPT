...
from core.inference.runtime import list_runtime_models, load_runtime_model
...
def list_available_models() -> list[str]:
    """List PyTorch checkpoint dirs and GGUF files."""
    models = list_runtime_models(PROJECT_ROOT)
...
def get_or_load_model(model_name: str):
    ...
    from core.inference.runtime import is_gguf_name

    if is_gguf_name(model_name) or model_name.endswith(".gguf"):
        try:
            model = load_runtime_model(model_name)
            log.model("loaded_gguf", model_name)
            _loaded_models[model_name] = model
            return model
        except Exception as e:
            log.error(f"Failed to load GGUF {model_name}", e)
            raise HTTPException(status_code=500, detail=f"Failed to load GGUF: {e}")
        ...
        if hasattr(torch, "compile") and model.config.device == "cuda":
            if not has_triton:
                ...
                try:
                    ...
                    model = model.compile_for_inference(mode="default")  # uses inductor under the hood
...
async def send_message(
    ...
    try:
        ...
        # =================================================================
        #
        # - "conversation": Simple Q&A, uses CHAT_SYSTEM_PROMPT
        #                   Matches Phase 3-4 chat SFT (no tools)
        # - "agentic": Uses AgentController with tool descriptions
        #              Requires Phase 5-6 training for best results
        ...
        if mode == "conversation":
            selected_system_prompt = CHAT_SYSTEM_PROMPT
